...
export default function CartPage() {
  ...
  // 4. PERBAIKAN: Mengemas SEMUA produk menjadi array sebelum dikirim ke checkout
  function handleGoToCheckout() {
    if (cartItems.length === 0) return;

    // Memetakan semua item di keranjang menjadi struktur array objek baru
    const itemsUntukCheckout = cartItems.map((item) => ({
      product_id: item.products.id, // ID Produk untuk relasi database order & stok
    ...
    // Alihkan pengguna ke file app/checkout/page.tsx yang baru
  ...
  return (
    <div className="bg-[#f8f9fa] min-h-screen text-gray-800 antialiased font-sans">
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        {cartItems.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-2xl border border-gray-200 shadow-xs p-6">
            ...
            <Link href="/" className="inline-block bg-gray-900 hover:bg-orange-500 text-white text-xs font-bold px-5 py-2.5 rounded-xl transition-all shadow-xs">
              Mulai Belanja
            </Link>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-xs overflow-hidden p-4 space-y-4">
              {cartItems.map((item) => (
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-xs p-5 space-y-4">
              ...
              <button 
                ...
                className="w-full bg-orange-500 hover:bg-orange-600 active:scale-[0.99] text-white font-bold py-3 rounded-xl text-xs flex items-center justify-center gap-2 transition-all shadow-xs"
              >
